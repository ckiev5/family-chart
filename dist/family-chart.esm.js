// https://donatso.github.io/family-chart/ v0.9.0 Copyright 2026 donatso
    ...
    otherFields.forEach((field) => {
      ...
      <div class="f3-form-field">
        ...
        <textarea name="${field.id}"
          placeholder="${escapeHtmlAttr(field.label)}">${escapeHtmlText(field.initial_value || "")}</textarea>
      </div>`;
        }
      ...
      <div class="f3-form-field">
        ...
        <select name="${select_field.id}">
          ...
          ${select_field.options
                .map((option) => {
                const selected = option.value === select_field.initial_value ? "selected" : "";
                return `<option ${selected} value="${escapeHtmlAttr(option.value)}">${escapeHtmlText(option.label)}</option>`;
            })
                .join("")}
        </select>
      </div>`;
    ...
    function infoField(fieldsList) {
        ...
        <div class="f3-info-field">
          ...
          <span class="f3-info-field-value">${escapeHtmlText(field.initial_value || "")}</span>
                ...
                const select_field = field;
                if (!select_field.initial_value)
                    return;
                const label = ((_a = select_field.options.find((option) => option.value === select_field.initial_value)) === null || _a === void 0 ? void 0 : _a.label) || "";
                fields_html += `
        <div class="f3-info-field">
